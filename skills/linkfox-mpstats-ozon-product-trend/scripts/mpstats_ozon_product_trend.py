#!/usr/bin/env python3
"""
MPSTATS Ozon Product Trend (Daily Time-Series) - LinkFox Skill
调用 /mpstats/ozon/productTrend 接口。

Usage:
  python mpstats_ozon_product_trend.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python mpstats_ozon_product_trend.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/mpstats/ozon/productTrend"
SLUG = "linkfox-mpstats-ozon-product-trend"
...
def main():
    ...
    if not argv:
        print(
            "Usage: mpstats_ozon_product_trend.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
