...
调用 /zhihuiya/patentImageSearch 接口。

Usage:
  python zhihuiya_patent_image_search.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python zhihuiya_patent_image_search.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
# 本 skill 仅支持实用新型专利：patentType 固定为 U，model 仅允许 3（匹配形状）/ 4（匹配形状/图案/色彩，推荐）
PATENT_TYPE = "U"
ALLOWED_MODELS = {3, 4}
DEFAULT_MODEL = 4
...
def main():
    ...
    if not argv:
        print(
            "Usage: zhihuiya_patent_image_search.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # 强制本 skill 支持的专利类型与模型，忽略/纠正入参中的实用新型专利以外的取值
    ...
    if model is None:
        ...
        print(
            f"本技能仅支持实用新型专利（model 必须为 3 或 4），收到 model={model}；"
            f"外观设计专利检索请使用 linkfox-zhihuiya-patent-image-search",
            file=sys.stderr,
        )
        sys.exit(1)
