#!/usr/bin/env python3
"""
Seerfar Ozon Product Detail Search - LinkFox Skill
调用 /seerfar/ozon/productDetailSearch 接口。

Usage:
  python seerfar_ozon_product_detail_search.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python seerfar_ozon_product_detail_search.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/seerfar/ozon/productDetailSearch"
SLUG = "linkfox-seerfar-ozon-product-detail-search"
...
def main():
    ...
    if not argv:
        print(
            "Usage: amazon_search.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
