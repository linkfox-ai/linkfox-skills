...
调用 /ruiguan/textTrademarkDetection 接口。

Usage:
  python ruiguan_text_trademark_detection.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python ruiguan_text_trademark_detection.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/ruiguan/textTrademarkDetection"
SLUG = "linkfox-ruiguan-text-trademark-detection"
...
def main():
    ...
    if not argv:
        print(
            "Usage: ruiguan_text_trademark_detection.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
