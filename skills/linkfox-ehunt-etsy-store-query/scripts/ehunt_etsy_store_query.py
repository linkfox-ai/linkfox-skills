#!/usr/bin/env python3
"""
EHunt Etsy 店铺查询（`_ehunt_storeQuery`） - LinkFox Skill
调用 /ehunt/etsy/storeQuery 接口。

Usage:
  python ehunt_etsy_store_query.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python ehunt_etsy_store_query.py '<JSON parameters>' --inline  # 强制全量打印到 stdout

输出策略（脚本默认行为）：
  - **始终**将完整响应写入 `<cwd>/linkfox/<YYYY-MM-DD>/<session>/data/linkfox-ehunt-etsy-store-query-<timestamp>.json`（`<cwd>` 为脚本执行时的工作目录，在 Claude Code 里即当前项目目录；`<session>` 取自环境变量 `SESSION_ID`，按用户任务自动聚合；**禁止写入 /tmp**，当前目录不可写则报错）
...
API_PATH = "/ehunt/etsy/storeQuery"
SLUG = "linkfox-ehunt-etsy-store-query"
...
def main():
    ...
    if not argv:
        print(
            "Usage: ehunt_etsy_store_query.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
