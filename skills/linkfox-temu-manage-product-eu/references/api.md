# linkfox-temu-manage-product-eu — API 参考

Temu **欧洲站商品管理**（Partner EU **Product > Manage Product**），经本 skill `temu_eu_proxy`（`POST /temu/proxy`） 转发。Temu 的 `type` 写在 Body，**不是** URL 路径。
...
发品（V2 add）请用 **`linkfox-temu-add-product-eu`**。价格/供货价请用 **`linkfox-temu-price-eu`**。订单请用 **`linkfox-temu-order-eu`**。电商税务请用 **`linkfox-temu-tax-eu`**。Self-Fulfilled Shipments（自发货）请用 **`linkfox-temu-fulfillment-eu`**。取消订单请用 **`linkfox-temu-cancel-order-eu`**。。
...
- `skillName`: **`linkfox-temu-manage-product-eu`**
