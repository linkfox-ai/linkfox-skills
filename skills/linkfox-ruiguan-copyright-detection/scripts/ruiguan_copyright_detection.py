...
调用 /ruiguan/copyrightDetection 接口。

Usage:
  python ruiguan_copyright_detection.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python ruiguan_copyright_detection.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/ruiguan/copyrightDetection"
SLUG = "linkfox-ruiguan-copyright-detection"
...
def main():
    ...
    if not argv:
        print(
            "Usage: ruiguan_copyright_detection.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
