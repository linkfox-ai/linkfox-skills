...
调用 /sorftime/amazon/productDetail 接口。

Usage:
  python sorftime_product_detail.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python sorftime_product_detail.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
SLUG = "linkfox-sorftime-amazon-product-detail"
...
def main():
    ...
    if not argv:
        print(
            "Usage: sorftime_product_detail.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
