# linkfox-temu-order-global — API 参考

Temu **全球站订单管理**（Partner Global **Order / 订单发货** 相关接口），经本 skill `temu_global_proxy`（`POST /temu/proxy`） 转发。Temu 的 `type` 写在 Body，**不是** URL 路径。

> 网关与鉴权：本 skill `scripts/`（`LINKFOXAGENT_API_KEY`、`accessToken` / `storeKey`）。订单场景 token 见 `references/access-token.md`（`tokenPurpose=order-shipping`）。（如未配置 按 SKILL.md 的 **## 解决认证和积分问题** 处理）
...
python scripts/global_order_list_v2_get.py '{"accessToken":"TOKEN","tokenPurpose":"order-shipping","request":{"pageNumber":1,"pageSize":20,"parentOrderStatus":2}}'
```
...
商品信息请用 **`linkfox-temu-manage-product-global`**；发品请用 **`linkfox-temu-add-product-us`**；取消订单请用 **`linkfox-temu-cancel-order-us`**；履约/发货请用 **`linkfox-temu-fulfillment-us`**。
...
- `skillName`: **`linkfox-temu-order-global`**
