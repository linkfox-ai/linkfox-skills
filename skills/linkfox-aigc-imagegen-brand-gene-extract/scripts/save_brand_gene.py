#!/usr/bin/env python3
"""品牌基因 JSON 落盘到会话 data/ 目录（linkfox_paths 协议）。"""
...
SLUG = "linkfox-aigc-imagegen-brand-gene-extract"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Save brandGeneJson to session data/")
    parser.add_argument(
        "--datadir",
        action="store_true",
        help="Print session data/ absolute path and exit (for intermediate params files)",
    )
    parser.add_argument(
        "input",
        nargs="?",
        help="JSON file to save; omit to read from stdin",
    )
    args = parser.parse_args()
    ts = time.time()

    if args.datadir:
        session = session_root(ts)
        print(os.path.abspath(os.path.join(session, "data")))
        return 0
    ...
    print(f"Saved full response: {os.path.abspath(out_path)} ({size_bytes} bytes)")
    return 0
