...
Usage:
  python mpstats_ozon_product_detail.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python mpstats_ozon_product_detail.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/mpstats/ozon/productDetail"
SLUG = "linkfox-mpstats-ozon-product-detail"
...
def main():
    ...
    if not argv:
        print(
            "Usage: mpstats_ozon_product_detail.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
