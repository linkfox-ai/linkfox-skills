"""Run any Shopee Product module API via developerProxy."""
...
from _product_endpoints import LIST_QUERY_FIELDS, PRODUCT_ENDPOINTS, RESERVED_PARAM_KEYS
from _shopee_product_common import (
...
def run_product_api(api_name: str, params: dict, caller: Optional[str] = None) -> dict:
    if api_name not in PRODUCT_ENDPOINTS:
        print(f"Unknown api: {api_name}. Valid: {', '.join(sorted(PRODUCT_ENDPOINTS))}", file=sys.stderr)
        sys.exit(1)

    spec = PRODUCT_ENDPOINTS[api_name]
