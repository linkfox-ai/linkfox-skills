...
调用 /amazon/opportunity/searchByMetrics 接口。

Usage:
  python amazon_opportunity_screener.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python amazon_opportunity_screener.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/amazon/opportunity/searchByMetrics"
SLUG = "linkfox-amazon-opportunity-search-by-metrics"
...
def main():
    ...
    if not argv:
        print(
            "Usage: amazon_opportunity_screener.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
