#!/usr/bin/env python3
"""
Zhihuiya Patent Bibliography - LinkFox Skill
调用 /zhihuiya/bibliography 接口。

Usage:
  python zhihuiya_bibliography.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python zhihuiya_bibliography.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/zhihuiya/bibliography"
...
def main():
    ...
    if not argv:
        print(
            "Usage: zhihuiya_bibliography.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
