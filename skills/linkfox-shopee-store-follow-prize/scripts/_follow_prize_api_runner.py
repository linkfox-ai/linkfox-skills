"""Run any Shopee Follow Prize module API via developerProxy."""
...
from _follow_prize_endpoints import LIST_QUERY_FIELDS, FOLLOW_PRIZE_ENDPOINTS, RESERVED_PARAM_KEYS
from _shopee_follow_prize_common import (
...
def run_follow_prize_api(api_name: str, params: dict, caller: Optional[str] = None) -> dict:
    if api_name not in FOLLOW_PRIZE_ENDPOINTS:
        print(f"Unknown api: {api_name}. Valid: {', '.join(sorted(FOLLOW_PRIZE_ENDPOINTS))}", file=sys.stderr)
        sys.exit(1)

    spec = FOLLOW_PRIZE_ENDPOINTS[api_name]
