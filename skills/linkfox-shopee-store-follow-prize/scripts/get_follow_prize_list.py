...
def main() -> None:
    if len(sys.argv) < 2:
        print("Usage: get_follow_prize_list.py '<JSON>'", file=sys.stderr)
        sys.exit(1)
    params = json.loads(sys.argv[1])
    inline = lf_inline_flag()
    emit_result(run_follow_prize_api("get_follow_prize_list", params, "get_follow_prize_list.py"), inline)
