#!/usr/bin/env python3
"""
Keepa Product Time-Series Data Explorer - LinkFox Skill
调用 /keepa/productSeries 接口。

Usage:
  python keepa_product_history.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python keepa_product_history.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
SLUG = "linkfox-keepa-product-series"
...
def main():
    ...
    if not argv:
        print(
            "Usage: keepa_product_history.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
