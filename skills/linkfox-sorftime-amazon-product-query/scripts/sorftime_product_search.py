...
调用 /sorftime/amazon/productQuery 接口。

Usage:
  python sorftime_product_search.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python sorftime_product_search.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/sorftime/amazon/productQuery"
SLUG = "linkfox-sorftime-amazon-product-query"
...
def main():
    ...
    if not argv:
        print(
            "Usage: sorftime_product_search.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
