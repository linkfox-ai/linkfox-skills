#!/usr/bin/env python3
"""
Ruiguan Policy Compliance Image Detection - LinkFox Skill
调用 /ruiguan/gunPartsSearch 接口。

Usage:
  python ruiguan_image_compliance_search.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python ruiguan_image_compliance_search.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/ruiguan/gunPartsSearch"
SLUG = "linkfox-ruiguan-gun-parts-search"
...
def main():
    ...
    if not argv:
        print(
            "Usage: ruiguan_image_compliance_search.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
