...
调用 /multimodal/generateImage 接口。

Usage:
  python multimodal_generate_image.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python multimodal_generate_image.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/multimodal/generateImage"
...
def main():
    ...
    if not argv:
        print(
            "Usage: multimodal_generate_image.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
