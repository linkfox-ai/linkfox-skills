#!/usr/bin/env python3
"""
ABA Data Explorer - LinkFox Skill
调用 /aba/intelligentQuery 接口。

Usage:
  python aba_query.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python aba_query.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
SLUG = "linkfox-aba-intelligent-query"
...
def main():
    ...
    if not argv:
        print(
            "Usage: aba_query.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
