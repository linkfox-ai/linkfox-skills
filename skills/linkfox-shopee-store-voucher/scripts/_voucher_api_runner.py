"""Run any Shopee Voucher module API via developerProxy."""
...
from _voucher_endpoints import LIST_QUERY_FIELDS, VOUCHER_ENDPOINTS, RESERVED_PARAM_KEYS
from _shopee_voucher_common import (
...
def run_voucher_api(api_name: str, params: dict, caller: Optional[str] = None) -> dict:
    if api_name not in VOUCHER_ENDPOINTS:
        print(f"Unknown api: {api_name}. Valid: {', '.join(sorted(VOUCHER_ENDPOINTS))}", file=sys.stderr)
        sys.exit(1)

    spec = VOUCHER_ENDPOINTS[api_name]
