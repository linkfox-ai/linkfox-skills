...
调用 /ruiguan/detectionPatentDesign 接口。

Usage:
  python ruiguan_detection_patent_design.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python ruiguan_detection_patent_design.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/ruiguan/detectionPatentDesign"
SLUG = "linkfox-ruiguan-detection-patent-design"
...
def main():
    ...
    if not argv:
        print(
            "Usage: ruiguan_detection_patent_design.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
