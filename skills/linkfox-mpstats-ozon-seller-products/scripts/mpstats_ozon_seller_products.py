...
调用 /mpstats/ozon/sellerProducts 接口。

Usage:
  python mpstats_ozon_seller_products.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python mpstats_ozon_seller_products.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/mpstats/ozon/sellerProducts"
...
def main():
    ...
    if not argv:
        print(
            "Usage: mpstats_ozon_seller_products.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
