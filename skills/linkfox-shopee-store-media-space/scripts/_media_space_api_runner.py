"""Run any Shopee MediaSpace module API via developerProxy."""
...
from _media_space_endpoints import LIST_QUERY_FIELDS, MEDIA_SPACE_ENDPOINTS, RESERVED_PARAM_KEYS
from _shopee_media_space_common import (
...
def run_media_space_api(api_name: str, params: dict, caller: Optional[str] = None) -> dict:
    if api_name not in MEDIA_SPACE_ENDPOINTS:
        print(f"Unknown api: {api_name}. Valid: {', '.join(sorted(MEDIA_SPACE_ENDPOINTS))}", file=sys.stderr)
        sys.exit(1)

    spec = MEDIA_SPACE_ENDPOINTS[api_name]
