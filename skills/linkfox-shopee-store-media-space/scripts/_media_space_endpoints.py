"""Shopee MediaSpace module API registry (v2.media_space.*)."""

from __future__ import annotations

from typing import Literal, TypedDict

SHOPEE_DOC_MODULE = "91"

class MediaSpaceEndpoint(TypedDict, total=False):
    path: str
...
MEDIA_SPACE_ENDPOINTS: dict[str, MediaSpaceEndpoint] = {
    "init_video_upload": {
    ...
    "upload_video_part": {
        ...
        "notes": 'Upload video chunk; may use multipart — pass body via developerProxy',
    },
...
def official_doc_url(api_name: str) -> str:
    return (
        f"https://open.shopee.com/documents/v2/v2.media_space.{api_name}"
        f"?module={SHOPEE_DOC_MODULE}&type=1"
    )

def list_api_names() -> list[str]:
    return sorted(MEDIA_SPACE_ENDPOINTS.keys())
