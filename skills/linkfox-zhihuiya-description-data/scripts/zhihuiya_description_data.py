#!/usr/bin/env python3
"""
Zhihuiya Patent Description Data - LinkFox Skill
调用 /zhihuiya/descriptionData 接口。

Usage:
  python zhihuiya_description_data.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python zhihuiya_description_data.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/zhihuiya/descriptionData"
...
def main():
    ...
    if not argv:
        print(
            "Usage: zhihuiya_description_data.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
