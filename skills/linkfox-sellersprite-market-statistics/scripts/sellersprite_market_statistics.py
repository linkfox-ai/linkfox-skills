...
调用 /sellersprite/market/statistics 接口。

Usage:
  python sellersprite_market_statistics.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python sellersprite_market_statistics.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/sellersprite/market/statistics"
SLUG = "linkfox-sellersprite-market-statistics"
...
def main():
    ...
    if not argv:
        print(
            "Usage: sellersprite_market_statistics.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
