...
description: 通过 `_ehunt_etsyCategorySearch`（MCP 展示名「Etsy品类查询」）在已同步到 MCP 库的 Etsy 类目数据中检索类目名称、id 与 parentIds，用于商品/店铺筛选的类目 id。当用户提到 Etsy 类目、Etsy category id、Etsy 类目树、_ehunt_etsyCategorySearch、Etsy品类查询、syncEtsyCategory / _ehunt_syncEtsyCategory（Etsy品类同步）后查询类目时触发。即使用户未提及工具名，只要在本地已同步的 Etsy 类目库里按关键词找类目 id，也应触发此技能。
---

# Etsy 类目检索（`_ehunt_etsyCategorySearch`）
