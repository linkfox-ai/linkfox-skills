"""Run any Shopee GlobalProduct module API via developerProxy."""
...
from _global_product_endpoints import LIST_QUERY_FIELDS, GLOBAL_PRODUCT_ENDPOINTS, RESERVED_PARAM_KEYS
from _shopee_global_product_common import (
...
def run_global_product_api(api_name: str, params: dict, caller: Optional[str] = None) -> dict:
    if api_name not in GLOBAL_PRODUCT_ENDPOINTS:
        print(f"Unknown api: {api_name}. Valid: {', '.join(sorted(GLOBAL_PRODUCT_ENDPOINTS))}", file=sys.stderr)
        sys.exit(1)

    spec = GLOBAL_PRODUCT_ENDPOINTS[api_name]
