...
调用 /sellersprite/competitor-lookup 接口。

Usage:
  python sellersprite_competitor_lookup.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python sellersprite_competitor_lookup.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/sellersprite/competitor-lookup"
SLUG = "linkfox-sellersprite-competitor-lookup"
...
def main():
    ...
    if not argv:
        print(
            "Usage: sellersprite_competitor_lookup.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
