...
调用 /tool-jungle-scout/keywords/share-of-voice 接口。

Usage:
  python junglescout_keyword_sov.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python junglescout_keyword_sov.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
def main():
    ...
    if not argv:
        print(
            "Usage: junglescout_keyword_sov.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
