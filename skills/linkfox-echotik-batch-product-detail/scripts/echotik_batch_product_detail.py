#!/usr/bin/env python3
"""
EchoTik TikTok Batch Product Detail - LinkFox Skill
调用 /echotik/batchProductDetail 接口。

Usage:
  python echotik_batch_product_detail.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python echotik_batch_product_detail.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/echotik/batchProductDetail"
SLUG = "linkfox-echotik-batch-product-detail"
...
def main():
    ...
    if not argv:
        print(
            "Usage: echotik_batch_product_detail.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
