"""Run any Shopee Public module API via developerProxy."""
...
from _public_endpoints import LIST_QUERY_FIELDS, PUBLIC_ENDPOINTS, RESERVED_PARAM_KEYS
from _shopee_public_common import (
...
def run_public_api(api_name: str, params: dict, caller: Optional[str] = None) -> dict:
    if api_name not in PUBLIC_ENDPOINTS:
        print(f"Unknown api: {api_name}. Valid: {', '.join(sorted(PUBLIC_ENDPOINTS))}", file=sys.stderr)
        sys.exit(1)

    spec = PUBLIC_ENDPOINTS[api_name]
