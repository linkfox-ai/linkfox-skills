...
调用 /alibaba1688/imageSearch 接口。

Usage:
  python alibaba1688_image_search.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python alibaba1688_image_search.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/alibaba1688/imageSearch"
SLUG = "linkfox-1688-search-by-image"
...
def main():
    ...
    if not argv:
        print(
            "Usage: alibaba1688_image_search.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
