#!/usr/bin/env python3
"""
FastMoss - TikTok Top Selling Rankings - LinkFox Skill
调用 /fastmoss/productRankTopSelling 接口。

Usage:
  python fastmoss_product_rank_top_selling.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python fastmoss_product_rank_top_selling.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
def main():
    ...
    if not argv:
        print(
            "Usage: fastmoss_product_rank_top_selling.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
