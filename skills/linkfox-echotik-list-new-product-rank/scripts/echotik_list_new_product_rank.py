#!/usr/bin/env python3
"""
EchoTik - TikTok New Product Ranking - LinkFox Skill
调用 /echotik/listNewProductRank 接口。

Usage:
  python echotik_list_new_product_rank.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python echotik_list_new_product_rank.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/echotik/listNewProductRank"
SLUG = "linkfox-echotik-list-new-product-rank"
...
def main():
    ...
    if not argv:
        print(
            "Usage: echotik_list_new_product_rank.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
