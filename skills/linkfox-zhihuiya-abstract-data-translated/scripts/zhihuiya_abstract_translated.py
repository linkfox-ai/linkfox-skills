...
调用 /zhihuiya/abstractDataTranslated 接口。

Usage:
  python zhihuiya_abstract_translated.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python zhihuiya_abstract_translated.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/zhihuiya/abstractDataTranslated"
...
def main():
    ...
    if not argv:
        print(
            "Usage: zhihuiya_abstract_translated.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
