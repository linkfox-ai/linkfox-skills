...
def main() -> None:
    if len(sys.argv) < 2:
        print("Usage: delete_shop_flash_sale_items.py '<JSON>'", file=sys.stderr)
        sys.exit(1)
    params = json.loads(sys.argv[1])
    inline = lf_inline_flag()
    emit_result(run_shop_flash_sale_api("delete_shop_flash_sale_items", params, "delete_shop_flash_sale_items.py"), inline)
