# SP-API Report Scripts Usage Guide

本目录包含 **报告获取** 相关的 Python 脚本。若需要授权、查看店铺或获取令牌，请使用依赖 skill `linkfox-amazon-spapi-auth` 的脚本。
...
- ⚠️ **依赖 skill `linkfox-amazon-spapi-auth` 已安装**（`get_report.py` 启动会自动校验）
...
探测依赖 skill `linkfox-amazon-spapi-auth` 是否可用。
...
1. 优先调用本地的 skill 安装工具（如有）自动安装 `linkfox-amazon-spapi-auth`
...
| reportType | string | Yes | SP-API report type |
...
| 42 | **DEPENDENCY_MISSING**：依赖 skill `linkfox-amazon-spapi-auth` 未安装 |
...
| SPAPI_BASE_URL | 后端网关 base URL | https://tool-gateway.linkfox.com |
...
- 安装依赖 skill `linkfox-amazon-spapi-auth`
