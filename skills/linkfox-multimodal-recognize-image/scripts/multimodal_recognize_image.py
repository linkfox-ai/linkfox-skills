#!/usr/bin/env python3
"""
Image Recognition - LinkFox Skill
调用 /multimodal/recognizeImage 接口。

Usage:
  python multimodal_recognize_image.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python multimodal_recognize_image.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/multimodal/recognizeImage"
...
def main():
    ...
    if not argv:
        print(
            "Usage: multimodal_recognize_image.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
