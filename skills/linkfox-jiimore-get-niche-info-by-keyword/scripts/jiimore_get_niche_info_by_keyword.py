...
调用 /jiimore/getNicheInfoByKeyword 接口。

Usage:
  python jiimore_get_niche_info_by_keyword.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python jiimore_get_niche_info_by_keyword.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/jiimore/getNicheInfoByKeyword"
SLUG = "linkfox-jiimore-get-niche-info-by-keyword"
...
def main():
    ...
    if not argv:
        print(
            "Usage: jiimore_get_niche_info_by_keyword.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
