"""Run any Shopee FirstMile module API via developerProxy."""
...
from _first_mile_endpoints import LIST_QUERY_FIELDS, FIRST_MILE_ENDPOINTS, RESERVED_PARAM_KEYS
from _shopee_first_mile_common import (
...
def run_first_mile_api(api_name: str, params: dict, caller: Optional[str] = None) -> dict:
    if api_name not in FIRST_MILE_ENDPOINTS:
        print(f"Unknown api: {api_name}. Valid: {', '.join(sorted(FIRST_MILE_ENDPOINTS))}", file=sys.stderr)
        sys.exit(1)

    spec = FIRST_MILE_ENDPOINTS[api_name]
