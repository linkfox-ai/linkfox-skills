...
调用 /tool-jungle-scout/product-database/query 接口。

Usage:
  python junglescout_product_database.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python junglescout_product_database.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/tool-jungle-scout/product-database/query"
...
def main():
    ...
    if not argv:
        print(
            "Usage: junglescout_product_database.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
