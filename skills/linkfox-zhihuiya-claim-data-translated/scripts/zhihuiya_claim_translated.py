...
调用 /zhihuiya/claimDataTranslated 接口。

Usage:
  python zhihuiya_claim_translated.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python zhihuiya_claim_translated.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/zhihuiya/claimDataTranslated"
...
def main():
    ...
    if not argv:
        print(
            "Usage: zhihuiya_claim_translated.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
