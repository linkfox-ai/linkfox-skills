"""Run any Shopee Discount module API via developerProxy."""
...
from _discount_endpoints import LIST_QUERY_FIELDS, DISCOUNT_ENDPOINTS, RESERVED_PARAM_KEYS
from _shopee_discount_common import (
...
def run_discount_api(api_name: str, params: dict, caller: Optional[str] = None) -> dict:
    if api_name not in DISCOUNT_ENDPOINTS:
        print(f"Unknown api: {api_name}. Valid: {', '.join(sorted(DISCOUNT_ENDPOINTS))}", file=sys.stderr)
        sys.exit(1)

    spec = DISCOUNT_ENDPOINTS[api_name]
