...
调用 /sellersprite/productSearch 接口。

Usage:
  python sellersprite_product_search.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python sellersprite_product_search.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
def main():
    ...
    if not argv:
        print(
            "Usage: sellersprite_product_search.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
