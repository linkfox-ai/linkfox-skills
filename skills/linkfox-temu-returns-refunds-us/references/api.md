# linkfox-temu-returns-refunds-us — API 参考

Temu **美国站退货与退款（Returns & Refunds）**，经本 skill `temu_us_proxy`（`POST /temu/proxy`） 转发。Temu 的 `type` 写在 Body，**不是** URL 路径。
...
| 能力 | skill | 典型场景 |
|------|--------|----------|
| 买家/卖家**取消订单**（未发货或取消申请） | `linkfox-temu-cancel-order-us` | `bg.aftersales.cancel.*`、`temu.order.cancel.*` |
| **退货、退款、售后退货退款** | **`linkfox-temu-returns-refunds-us`**（本 skill） | Partner **Returns & Refunds** 菜单下 9 条接口（见 [apis/README.md](./apis/README.md)） |

订单上下文（`parentOrderSn`、`orderSn`、售后单号等）通常先通过 **`linkfox-temu-order-us`** 获取。
...
```text
1. linkfox-temu-order-us                              → 订单/售后上下文
...
- `skillName`: **`linkfox-temu-returns-refunds-us`**
