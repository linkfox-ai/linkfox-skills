#!/usr/bin/env python3
"""
Amazon Policy & Regulation Feed - LinkFox Skill
调用 /amazon/policyFeedDetail 接口。

Usage:
  python amazon_policy_feed_detail.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python amazon_policy_feed_detail.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/amazon/policyFeedDetail"
SLUG = "linkfox-amazon-policy-feed"
...
def main():
    ...
    if not argv:
        print(
            "Usage: amazon_policy_feed_detail.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
