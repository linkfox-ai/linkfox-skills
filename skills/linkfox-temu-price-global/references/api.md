# linkfox-temu-price-global — API 参考

Temu **全球站商品价格管理**（Partner Global **Product** 下价格/供货价相关接口），经本 skill `temu_global_proxy`（`POST /temu/proxy`） 转发。Temu 的 `type` 写在 Body，**不是** URL 路径。
...
python scripts/global_price_priceorder_query.py '{"accessToken":"TOKEN","request":{"page":1,"size":20,"priceOrderType":1}}'
```
...
```text
1. linkfox-temu-manage-product-global → list/detail / spec.info.get           → 拿 goodsId、catId、specIdList
...
- `skillName`: **`linkfox-temu-price-global`**
