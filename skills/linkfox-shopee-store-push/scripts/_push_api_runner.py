"""Run any Shopee Push module API via developerProxy."""
...
from _push_endpoints import LIST_QUERY_FIELDS, PUSH_ENDPOINTS, RESERVED_PARAM_KEYS
from _shopee_push_common import (
...
def run_push_api(api_name: str, params: dict, caller: Optional[str] = None) -> dict:
    if api_name not in PUSH_ENDPOINTS:
        print(f"Unknown api: {api_name}. Valid: {', '.join(sorted(PUSH_ENDPOINTS))}", file=sys.stderr)
        sys.exit(1)

    spec = PUSH_ENDPOINTS[api_name]
