#!/usr/bin/env python3
"""
Zhihuiya Patent Citations Explorer - LinkFox Skill
调用 /zhihuiya/patentCited 接口。

Usage:
  python zhihuiya_cited_by.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python zhihuiya_cited_by.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
def main():
    ...
    if not argv:
        print(
            "Usage: zhihuiya_cited_by.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
