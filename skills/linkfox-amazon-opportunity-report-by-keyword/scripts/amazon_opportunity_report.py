...
调用 /amazon/opportunity/reportByKeyword 接口。

Usage:
  python amazon_opportunity_report.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python amazon_opportunity_report.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
SLUG = "linkfox-amazon-opportunity-report-by-keyword"
...
def main():
    ...
    if not argv:
        print(
            "Usage: amazon_opportunity_report.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
