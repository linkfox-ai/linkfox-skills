#!/usr/bin/env python3
"""
Google Trends Time-Range Analysis - LinkFox Skill
调用 /googleTrend/getTrendByTime 接口。

Usage:
  python google_trends_rising.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python google_trends_rising.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/googleTrend/getTrendByTime"
SLUG = "linkfox-google-trend-get-trend-by-time"
...
def main():
    ...
    if not argv:
        print(
            "Usage: google_trends_rising.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
