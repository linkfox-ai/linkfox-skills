#!/usr/bin/env python3
"""
EchoTik TikTok Video Download - LinkFox Skill
调用 /echotik/getVideoDownloadUrl 接口。

Usage:
  python echotik_get_video_download_url.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python echotik_get_video_download_url.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/echotik/getVideoDownloadUrl"
SLUG = "linkfox-echotik-get-video-download-url"
...
def main():
    ...
    if not argv:
        print(
            "Usage: echotik_get_video_download_url.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
