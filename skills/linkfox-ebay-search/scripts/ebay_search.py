...
Usage:
  python ebay_search.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python ebay_search.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/ebay/search"
SLUG = "linkfox-ebay-search"
...
def main():
    ...
    if not argv:
        print(
            "Usage: ebay_search.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
