...
调用 /sif/keywordOverview 接口。

Usage:
  python sif_keyword_overview.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python sif_keyword_overview.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
def main():
    ...
    if not argv:
        print(
            "Usage: sif_keyword_overview.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
