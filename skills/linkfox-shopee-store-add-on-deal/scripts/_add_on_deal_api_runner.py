"""Run any Shopee Add-On Deal module API via developerProxy."""
...
from _add_on_deal_endpoints import LIST_QUERY_FIELDS, ADD_ON_DEAL_ENDPOINTS, RESERVED_PARAM_KEYS
from _shopee_add_on_deal_common import (
...
def run_add_on_deal_api(api_name: str, params: dict, caller: Optional[str] = None) -> dict:
    if api_name not in ADD_ON_DEAL_ENDPOINTS:
        print(f"Unknown api: {api_name}. Valid: {', '.join(sorted(ADD_ON_DEAL_ENDPOINTS))}", file=sys.stderr)
        sys.exit(1)

    spec = ADD_ON_DEAL_ENDPOINTS[api_name]
