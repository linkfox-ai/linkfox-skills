#!/usr/bin/env python3
"""
SP Keywords List - linkfox-amazon-ads-manager
============================================

POST sp/keywords/list (Amazon Ads v3).

Usage:
  python list_keywords.py '<JSON params>'

Required:
  profileId (number), region (NA/EU/FE)

Optional filters:
  keywordIdFilter      {"include": ["...", ...]}
  adGroupIdFilter      {"include": ["...", ...]}
  campaignIdFilter     {"include": ["...", ...]}
  stateFilter          {"include": ["ENABLED", "PAUSED", "ARCHIVED"]}
  matchTypeFilter      {"include": ["BROAD", "PHRASE", "EXACT"]}
...
Example:
  python list_keywords.py '{"profileId": 1234567890, "region": "NA",
                            "campaignIdFilter": {"include": ["998877"]},
                            "matchTypeFilter": {"include": ["EXACT"]}}'
"""
...
FILTER_KEYS = [
    ...
    "campaignIdFilter",
    "stateFilter",
    "matchTypeFilter",          # 裸数组，如 ["EXACT","BROAD"]
    "keywordTextFilter",        # {"queryTermMatchType":"BROAD_MATCH","include":["soap"]}
]
...
def main() -> None:
    ...
    print(
        f"\n✓ Fetched {len(items)} keywords across {output['pagesFetched']} page(s)"
