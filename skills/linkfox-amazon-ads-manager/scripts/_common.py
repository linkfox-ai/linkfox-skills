"""
Shared helpers for linkfox-amazon-ads-manager scripts.
...
def ensure_auth_skill_available() -> None:
    ...
    if not checker.exists():
        payload = {
            ...
            "suggestedActions": [
                f"Install skill '{REQUIRED_SKILL}' before running linkfox-amazon-ads-manager.",
            ],
        }
...
def get_api_key() -> str:
    ...
    if not key:
        print(
            "❌ LINKFOX_AGENT_API_KEY not configured. Please set:\n"
            "  export LINKFOX_AGENT_API_KEY=your-key-here",
            file=sys.stderr,
        )
        sys.exit(1)
    return key
