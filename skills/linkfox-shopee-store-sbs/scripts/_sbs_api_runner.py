"""Run any Shopee SBS module API via developerProxy."""
...
from _sbs_endpoints import LIST_QUERY_FIELDS, SBS_ENDPOINTS, RESERVED_PARAM_KEYS
from _shopee_sbs_common import (
...
def run_sbs_api(api_name: str, params: dict, caller: Optional[str] = None) -> dict:
    if api_name not in SBS_ENDPOINTS:
        print(f"Unknown api: {api_name}. Valid: {', '.join(sorted(SBS_ENDPOINTS))}", file=sys.stderr)
        sys.exit(1)

    spec = SBS_ENDPOINTS[api_name]
