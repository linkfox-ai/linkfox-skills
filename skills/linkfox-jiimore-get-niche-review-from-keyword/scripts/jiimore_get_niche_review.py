...
调用 /jiimore/getNicheReviewFromKeyword 接口。

Usage:
  python jiimore_get_niche_review.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python jiimore_get_niche_review.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
SLUG = "linkfox-jiimore-get-niche-review-from-keyword"
...
def main():
    ...
    if not argv:
        print(
            "Usage: jiimore_get_niche_review.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
