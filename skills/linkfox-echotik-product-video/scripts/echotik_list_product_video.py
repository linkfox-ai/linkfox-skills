...
调用 /echotik/listProductVideo 接口。

Usage:
  python echotik_list_product_video.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python echotik_list_product_video.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/echotik/listProductVideo"
SLUG = "linkfox-echotik-product-video"
...
def main():
    ...
    if not argv:
        print(
            "Usage: echotik_list_product_video.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
