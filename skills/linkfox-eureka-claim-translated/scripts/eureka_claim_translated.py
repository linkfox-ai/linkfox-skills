#!/usr/bin/env python3
"""
Eureka Patent Claim Translation - LinkFox Skill
调用 /tool-eureka/claimDataTranslated 接口。

Usage:
  python eureka_claim_translated.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python eureka_claim_translated.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/tool-eureka/claimDataTranslated"
SLUG = "linkfox-eureka-claim-translated"
...
def main():
    ...
    if not argv:
        print(
            "Usage: eureka_claim_translated.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
