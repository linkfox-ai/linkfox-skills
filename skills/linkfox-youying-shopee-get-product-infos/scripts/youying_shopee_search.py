...
调用 /youying/shopee/getProductInfos 接口。

Usage:
  python youying_shopee_search.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python youying_shopee_search.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/youying/shopee/getProductInfos"
SLUG = "linkfox-youying-shopee-get-product-infos"
...
def main():
    ...
    if not argv:
        print(
            "Usage: youying_shopee_search.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
