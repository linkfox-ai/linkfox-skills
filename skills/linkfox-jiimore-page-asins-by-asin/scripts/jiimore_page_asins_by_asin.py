...
调用 /jiimore/pageAsinsByAsin 接口。

Usage:
  python jiimore_page_asins_by_asin.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python jiimore_page_asins_by_asin.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/jiimore/pageAsinsByAsin"
SLUG = "linkfox-jiimore-page-asins-by-asin"
...
def main():
    ...
    if not argv:
        print(
            "Usage: jiimore_page_asins_by_asin.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
