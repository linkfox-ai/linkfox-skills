#!/usr/bin/env python3
"""
Zhihuiya Patent Legal Status - LinkFox Skill
调用 /zhihuiya/legalStatus 接口。

Usage:
  python zhihuiya_legal_status.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python zhihuiya_legal_status.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
def main():
    ...
    if not argv:
        print(
            "Usage: zhihuiya_legal_status.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
