#!/usr/bin/env python3
"""
SIF ASIN Keyword Analysis - LinkFox Skill
调用 /sif/asinKeywords 接口。

Usage:
  python sif_asin_keywords.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python sif_asin_keywords.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/sif/asinKeywords"
SLUG = "linkfox-sif-asin-keywords"
...
def main():
    ...
    if not argv:
        print(
            "Usage: sif_asin_keywords.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
