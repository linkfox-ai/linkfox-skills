# linkfox-temu-promotion-global — API 参考

Temu **全球站电商促销**（Partner Global **Promotion** / 促销活动等），经本 skill `temu_global_proxy`（`POST /temu/proxy`） 转发。Temu 的 `type` 写在 Body，**不是** URL 路径。
...
| 能力 | skill |
|------|--------|
| **促销**（本 skill） | **`linkfox-temu-promotion-global`** |
| 商品管理（列表/编辑/库存/上下架） | `linkfox-temu-manage-product-global` |
...
- `skillName`: **`linkfox-temu-promotion-global`**
