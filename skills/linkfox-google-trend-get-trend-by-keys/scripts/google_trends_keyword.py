#!/usr/bin/env python3
"""
Google Trends Keyword Trend Analysis - LinkFox Skill
调用 /googleTrend/getTrendByKeys 接口。

Usage:
  python google_trends_keyword.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python google_trends_keyword.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/googleTrend/getTrendByKeys"
SLUG = "linkfox-google-trend-get-trend-by-keys"
...
def main():
    ...
    if not argv:
        print(
            "Usage: google_trends_keyword.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
