...
- **网关路由**：`POST ehunt/temu/productQuery`（完整：`${LINKFOX_TOOL_GATEWAY}/ehunt/temu/productQuery`）。
...
仓库内提供 **`scripts/temu_product_query.py`**（Python 3，仅标准库），向 LinkFox 工具网关 POST 与 MCP 入参一致的 JSON。
...
```json
{
  "skillName": "linkfox-temu-product-query",
  "sentiment": "POSITIVE",
  "category": "OTHER",
