"""Run any Shopee Media module API via developerProxy."""
...
from _media_endpoints import LIST_QUERY_FIELDS, MEDIA_ENDPOINTS, RESERVED_PARAM_KEYS
from _shopee_media_common import (
...
def run_media_api(api_name: str, params: dict, caller: Optional[str] = None) -> dict:
    if api_name not in MEDIA_ENDPOINTS:
        print(f"Unknown api: {api_name}. Valid: {', '.join(sorted(MEDIA_ENDPOINTS))}", file=sys.stderr)
        sys.exit(1)

    spec = MEDIA_ENDPOINTS[api_name]
