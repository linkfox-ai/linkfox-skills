# linkfox-temu-price-us — API 参考

Temu **美国站商品价格管理**（Partner US **Product** 下价格/供货价相关接口），经本 skill `temu_us_proxy`（`POST /temu/proxy`） 转发。Temu 的 `type` 写在 Body，**不是** URL 路径。
...
python scripts/us_price_priceorder_query.py '{"accessToken":"TOKEN","request":{"page":1,"size":20,"priceOrderType":1}}'
```
...
```text
1. linkfox-temu-manage-product-us → list/detail / spec.info.get           → 拿 goodsId、catId、specIdList
...
- `skillName`: **`linkfox-temu-price-us`**
