# SP-API Authorization API Reference

本文档描述 **授权与店铺/令牌管理** 相关的 API。若需要 SP-API 代理调用（如拉取报告），请参考 `linkfox-amazon-spapi-report` skill。
...
返回的 `accessToken` 可交给下游 skill（如 `linkfox-amazon-spapi-report`）用于具体 SP-API 调用。
...
```json
{
  "skillName": "linkfox-amazon-spapi-auth",
  "sentiment": "POSITIVE",
  "category": "OTHER",
